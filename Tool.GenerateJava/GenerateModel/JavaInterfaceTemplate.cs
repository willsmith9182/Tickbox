...
#line 25 "D:\Lat Projects\WL_Main\Tool.GenerateJava\GenerateModel\JavaInterfaceTemplate.tt"
            Write(ToStringHelper.ToStringWithCulture(interfaceName));
...
#line hidden
        ...
        public virtual void Initialize()
        {
            if ((Errors.HasErrors == false))
            {
                ...
                var interfaceNameValueAcquired = false;
                if (Session.ContainsKey("interfaceName"))
                {
                    interfaceName = ((string) (Session["interfaceName"]));
                    interfaceNameValueAcquired = true;
                }
                if ((interfaceNameValueAcquired == false))
                {
                    var data = CallContext.LogicalGetData("interfaceName");
                    if ((data != null))
                    {
