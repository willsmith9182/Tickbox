...
{
    class EnumPGen : IDatatypeGenerator
    {
        ...
        public IEnumerable<string> GenerateTModelImports(string sourceNamespace, List<string> relativeNamespace, string dtoPackage, string destTModelPackage)
        {
            ...
            yield return string.Format("{0}.{1}", dtoPackage + string.Join("", DtGenUtil.CalculateRelativeNamespace((_prop.PropType.Namespace ?? ""), sourceNamespace).Select(n => "." + n.ToLowerInvariant())), _prop.PropType.Name);

        }

        public IEnumerable<string> GenerateTModelProperties(string sourceNamespace, GenClass genClass)
        {
            yield return string.Format("\tpublic final EnumProperty<{1}> {0} = enumProperty(\"{0}\");", DtGenUtil.ToJavaMemberName(_prop.Name), _prop.PropType.Name);
        }
