...
{
    internal class EnumArrayPGen : IDatatypeGenerator
    {
        ...
        public IEnumerable<string> GeneratePropertyMethods(string sourceNamespace, GenClass genClass)
        {
            ...
            yield return
                DtGenUtil.GenNativeSetMethod(_prop, "JsArrayInteger", false, string.Format("set{0}Raw", _prop.Name),
                    genClass);
            ...
            if (_prop.CanWrite)
            {
                yield return
                    string.Format(
                        "\tpublic final {2} set{1}(Iterable<{0}> val) {{ set{1}Raw(JsArrayIntegerWrapper.from(JavaOnlyUtils.select(val, new Func<{0}, Integer>() {{ @Override public Integer call({0} val) {{ return val.getValue(); }}}})));	return this; }}",
                        _prop.PropType.GenericTypeArguments[0].Name, _prop.Name, genClass.Name);
            }
        }
            ...
            if (_prop.CanRead)
            {
                yield return
                    DtGenUtil.GenInterfaceGetMethod(_prop,
                        string.Format("List<{0}>", _prop.PropType.GenericTypeArguments[0].Name));
            }
            if (_prop.CanWrite)
            {
                yield return
                    DtGenUtil.GenInterfaceSetMethod(_prop,
                        string.Format("Iterable<{0}>", _prop.PropType.GenericTypeArguments[0].Name), genClass);
            }
        }
        ...
        public IEnumerable<string> GenerateStubPropertyMethods(string sourceNamespace, GenClass genClass)
        {
            yield return
                DtGenUtil.GenStubPrivateMember(_prop,
                    string.Format("List<{0}>", _prop.PropType.GenericTypeArguments[0].Name),
                    string.Format("new ArrayList<{0}>()", _prop.PropType.GenericTypeArguments[0].Name));
            yield return
            ...
            yield return
                string.Format(
                    "\t@Override public I{2} set{0}(Iterable<{1}> val) {{ this._{0} = JavaOnlyUtils.toList(val); return this; }}",
...
//            yield return "org.tessell.model.validation.rules.Required";
        ...
        {
            yield return
                string.Format("\tpublic final ListProperty<{1}> {0} = listProperty(\"{0}\");",
