...
{
    class NullNumberPGen : IDatatypeGenerator
    {
        ...
        public IEnumerable<string> GeneratePropertyMethods(string sourceNamespace, GenClass genClass)
        {
            ...
            yield return DtGenUtil.GenNativeSetMethod(_prop, "double", false, string.Format("set{0}Raw", _prop.Name), genClass);
...
//            yield return "org.tessell.model.validation.rules.Required";
        ...
        {
            yield return string.Format("\tpublic final NumberProperty {0} = new NumberProperty(new SetValue<Double>(\"{0}\"));", DtGenUtil.ToJavaMemberName(_prop.Name));
        }
...
//            yield return string.Format("{0}.addRule(new Required(\"required field\"));", DtGenUtil.ToJavaMemberName(_prop.Name));
        ...
        {
            yield return string.Format("\t\tresult.set{1}({0}.get() == null ? NullableDouble.getNull() : new NullableDouble({0}.get()));", DtGenUtil.ToJavaMemberName(_prop.Name), _prop.Name);
        }
    }
}
