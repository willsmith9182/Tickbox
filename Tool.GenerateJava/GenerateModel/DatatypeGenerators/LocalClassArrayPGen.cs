...
{
    class LocalClassArrayPGen : IDatatypeGenerator
    {
        ...
        public IEnumerable<string> GenerateImports(string sourceNamespace, List<string> myNamespaceList, string destPackage)
        {
            ...
            if ((_prop.PropType.GenericTypeArguments[0].Namespace ?? "") != myNamespace)
            {
                ...
                yield return string.Format("{0}.{1}", destPackage + string.Join("", DtGenUtil.CalculateRelativeNamespace((_prop.PropType.GenericTypeArguments[0].Namespace ?? ""), sourceNamespace).Select(n => "." + n.ToLowerInvariant())), "I" + _prop.PropType.GenericTypeArguments[0].Name);
            }
        }
        ...
        {
            yield return DtGenUtil.GenSetCall(_prop, string.Format("Utils.<{0}>createArray()", _prop.PropType.GenericTypeArguments[0].Name));
        }

        public IEnumerable<string> GenerateConvertDates()
        {
            yield return
                string.Format("for({1} o : new JsIterable<{1}>(get{0}Raw())) {{ o.convertDates(); }}", _prop.Name, _prop.PropType.GenericTypeArguments[0].Name);
        }
        ...
        public IEnumerable<string> GenerateTModelImports(string sourceNamespace, List<string> relativeNamespace, string dtoPackage, string destTModelPackage)
        {
            ...
            yield return string.Format("{0}.{1}", dtoPackage + string.Join("", DtGenUtil.CalculateRelativeNamespace((_prop.PropType.GenericTypeArguments[0].Namespace ?? ""), sourceNamespace).Select(n => "." + n.ToLowerInvariant())), "I" + _prop.PropType.GenericTypeArguments[0].Name);
            yield return string.Format("{0}.{1}", destTModelPackage + string.Join("", DtGenUtil.CalculateRelativeNamespace((_prop.PropType.GenericTypeArguments[0].Namespace ?? ""), sourceNamespace).Select(n => "." + n.ToLowerInvariant())), _prop.PropType.GenericTypeArguments[0].Name + "Tm");

        }

        public IEnumerable<string> GenerateTModelProperties(string sourceNamespace, GenClass genClass)
        {
            yield return string.Format("\tpublic final ListProperty<{1}Tm> {0} = listProperty(\"{0}\");", DtGenUtil.ToJavaMemberName(_prop.Name), _prop.PropType.GenericTypeArguments[0].Name);
        }
