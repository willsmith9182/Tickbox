...
{
    class NullEnumPGen : IDatatypeGenerator
    {
        ...
        public IEnumerable<string> GenerateImports(string sourceNamespace, List<string> myNamespaceList, string destPackage)
        {
            ...
            {
                yield return
                    string.Format("{0}.{1}", destPackage + string.Join("", DtGenUtil.CalculateRelativeNamespace((_prop.PropType.GenericTypeArguments[0].Namespace ?? ""), sourceNamespace).Select(n => "." + n.ToLowerInvariant())),
        ...
        {
            yield return DtGenUtil.GenInterfaceGetMethod(_prop, string.Format("NullableEnum<{0}>", _prop.PropType.GenericTypeArguments[0].Name));
            yield return DtGenUtil.GenInterfaceSetMethod(_prop, string.Format("NullableEnum<{0}>", _prop.PropType.GenericTypeArguments[0].Name), genClass);
        }
